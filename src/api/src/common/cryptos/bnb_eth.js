...
const ETH_CONTRACT_ADDRESS = process.env.APP_MOD === 'dev'
  ? '0x8babbb98678facc7342735486c851abd7a0d17ca' // BSC Testnet ETH (Mock)
  : '0x2170Ed0880ac9A755fd29B2688956BD959F933F8'; // BSC Mainnet ETH (BEP-20)
...
class InsufficientTokenBalanceError extends Error {
  constructor(balance, amount) {
    super(`Insufficient ETH balance. Available: ${balance}, Required: ${amount}`);
...
const getBalance = async (walletAddress) => {
  ...
  try {
    ...
    return parseFloat(ethers.formatUnits(balance, 18)); // ETH uses 18 decimals
  } catch (error) {
    throw new TransactionError(`ETH balance check failed: ${error.message}`);
  }
};
...
const transferFunds = async (
  ...
  try {
    ...
    const contract = new ethers.Contract(ETH_CONTRACT_ADDRESS, ETH_ABI, wallet);
    ...
    // Convert amount to ETH units (18 decimals)
    const amount = ethers.parseUnits(amountETH.toString(), 18);
    ...
    throw new TransactionError(
      ...
      error instanceof InsufficientTokenBalanceError
        ? error.message
        : `ETH transfer failed: ${error.message}`
    );
  }
};
...
const getTransactionStatus = async (txHash) => {
  try {
    ...
    const transferEvents = receipt.logs
      .filter(log => log.address.toLowerCase() === ETH_CONTRACT_ADDRESS.toLowerCase())
      .map(log => {
        try {
