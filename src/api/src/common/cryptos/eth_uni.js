...
const UNI_CONTRACT_ADDRESS = process.env.APP_MOD === 'dev'
  ? '0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984' // Sepolia testnet UNI (mock)
  : '0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984'; // Mainnet UNI
...
class InsufficientTokenBalanceError extends Error {
  constructor(balance, amount) {
    super(`Insufficient UNI balance. Available: ${balance}, Required: ${amount}`);
...
/**
 * Get UNI balance for a given address
 * @param {string} walletAddress - Ethereum wallet address
 * @returns {number} Balance in UNI
 */
const getBalance = async (walletAddress) => {
  ...
  try {
    const contract = new ethers.Contract(UNI_CONTRACT_ADDRESS, UNI_ABI, PROVIDER);
    const balance = await contract.balanceOf(walletAddress);
    return parseFloat(ethers.formatUnits(balance, 18)); // UNI uses 18 decimals
  } catch (error) {
    throw new TransactionError(`UNI balance check failed: ${error.message}`);
  }
};
...
/**
 * Transfer UNI between addresses
 ...
 * @param {number} amountUNI - Amount to send in UNI
 * @returns {string} Transaction hash
 */
const transferFunds = async (
  ...
  try {
    ...
    const contract = new ethers.Contract(UNI_CONTRACT_ADDRESS, UNI_ABI, wallet);
    ...
    // Convert amount to UNI units (18 decimals)
    const amount = ethers.parseUnits(amountUNI.toString(), 18);
