...
const arbMiddleware = require('./arb_eth');
...
const NETWORK_CONFIG = {
  mainnet: {
    ...
    tokenAddress: '0x912CE59144191C1204E64559FE8253a0e49E6548', // Native ARB on Arbitrum
    chainId: 42161,
    gasLimit: 100000n,
    minGasBalance: ethers.parseUnits('0.000002', 'ether'),
  ...
  testnet: {
    ...
    tokenAddress: '0x7Cc1d0980c8D0e248e4F94cf3714F890Dc4F084c', // Testnet ARB address
    chainId: 421614,
    gasLimit: 150000n,
...
const getBalance = async (walletAddress) => {
    ...
    try {
      ...
      activeProvider = createProvider();
      throw new TransactionError(`ARB balance check failed: ${error.message}`);
    }
};

const transferFunds = async (senderAddress, senderPrivateKey, receiverAddress, amountARB) => {
    try {
      ...
      const amountWei = ethers.parseUnits(amountARB.toString(), CONFIG.tokenDecimals);
      ...
      activeProvider = createProvider();
      throw new TransactionError(`ARB transfer failed: ${error.message}`);
    }
};
...
const needAlimentation = async (walletAddress) => {
  try {
    ...
    const ethBalance = await arbMiddleware.getBalance(walletAddress);
...
const alimentGasFees = async (targetAddress, amountETH) => {
  try {
    ...
    const targetNetwork = await Network.findOne({ symbol: 'ARB' })
    if (!targetNetwork) {
    ...
    return await arbMiddleware.transferFunds(
      adminWallet.address,
