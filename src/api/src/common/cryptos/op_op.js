...
const NETWORK_CONFIG = {
  mainnet: {
    ...
    tokenAddress: '0x4200000000000000000000000000000000000042', // OP on op mainnet
    chainId: 10,
    gasLimit: 100000n,
  ...
  testnet: {
    ...
    tokenAddress: '0x4200000000000000000000000000000000000042', // Testnet OP address
    chainId: 11155420,
    gasLimit: 150000n,
...
class InsufficientTokenBalanceError extends Error {
  constructor(balance, amount) {
    super(`Insufficient OP balance. Available: ${balance}, Required: ${amount}`);
...
const getBalance = async (walletAddress) => {
    ...
    try {
      ...
      activeProvider = createProvider();
      throw new TransactionError(`OP balance check failed: ${error.message}`);
    }
};

const transferFunds = async (senderAddress, senderPrivateKey, receiverAddress, amountOP) => {
    try {
      ...
      const amountWei = ethers.parseUnits(amountOP.toString(), CONFIG.tokenDecimals);
      ...
      activeProvider = createProvider();
      throw new TransactionError(`OP transfer failed: ${error.message}`);
    }
};
