...
const USDT_CONTRACT_ADDRESS = process.env.APP_MOD === 'dev'
  ? '0x78ae2880bd1672b49a33cF796CF53FE6db0aB01D' // Fuji testnet USDT (mock)
  : '0x9702230A8Ea53601f5cD2dc00fDBc13d4dF4A8c7'; // Avalanche C-Chain USDT
...
const getBalance = async (walletAddress) => {
  ...
  try {
    ...
    return parseFloat(ethers.formatUnits(balance, 6)); // USDT on Avalanche uses 6 decimals
  } catch (error) {
...
const alimentGasFees = async (targetAddress, amountAVAX) => {
  try {
    ...
    // Calculate exact gas needed for the USDT transfer
 ...
 * @param {number} amountUSDT - Amount to send in USDT
 * @returns {string} Transaction hash
 */
