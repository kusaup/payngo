...
const NETWORK_CONFIG = {
  mainnet: {
    ...
    usdtAddress: ethers.getAddress('0xc2132D05D31c914a87C6611C10748AEb04B58e8F'),
    chainId: 137,
    derivationPath: "m/44'/966'/0'/0/0",
    erc20GasLimit: 200000, // Gas limit for USDT transfers
  ...
  testnet: {
    ...
    usdtAddress: ethers.getAddress('0xbCF39d8616d15FD146dd5dB4a86b4f244A9Bc772'), // Verified Amoy USDT
    chainId: 80002,
...
initializeProvider().then(provider => {
  ...
  USDT_CONTRACT = new ethers.Contract(
    CONFIG.usdtAddress,
    USDT_CONTRACT.interface.format(),
    provider
  );
}).catch(error => {
...
const getBalance = async (walletAddress) => {
  ...
  try {
    const balance = await USDT_CONTRACT.balanceOf(walletAddress);
...
// pol_usdt.js - Updated transferFunds function with gas price escalation and retry mechanism
const transferFunds = async (senderAddress, senderPrivateKey, receiverAddress, amountUSDT) => {
  ...
  while (attempt < MAX_RETRIES) {
    try {
      ...
      const connectedContract = USDT_CONTRACT.connect(wallet);

      // Convert amount first
      const amountWei = ethers.parseUnits(amountUSDT.toString(), 6);
